...
function [r_high_res,E,E_high_res,J] = hp_calc_electric_field_1_5D_all_orders_flags(profiles,flags,settings)
...
%     indices_z = mod(indices,3)==2;
    ...
    if(aperature)
        ...
        if(weak_form)
            rhs_r = -exp(-1i*kx_test*(r0+1.10-x_L))*1i*real(omega)*mu0*[0 1 0];
        else
...
%      DIE
%     
    clearvars -except matrix rhs N de_alias N_high_res L kx_test x x_L x_R kn;
...
%     A = STOP
    ...
    figure(21)
    plot_components_and_abs(kn,abs(E_spectrum),'-',get(gca,'colororder'));
    xlabel('n');
    ylabel('|En|');
